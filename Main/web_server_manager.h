...
// ============ CHAT SENTIMENT DETECTION ============
// Detects emotion from user's chat message based on keywords
Emotion detectChatSentiment(String msg) {
  msg.toLowerCase();

  // Positive/Love keywords (Bengali + English)
  if (msg.indexOf("love") != -1 || msg.indexOf("ভালোবাসি") != -1 ||
      msg.indexOf("cute") != -1 || msg.indexOf("sweet") != -1 ||
      msg.indexOf("beautiful") != -1 || msg.indexOf("সুন্দর") != -1 ||
      msg.indexOf("পছন্দ") != -1 || msg.indexOf("ভাল লাগে") != -1 ||
      msg.indexOf("amazing") != -1 || msg.indexOf("দারুণ") != -1 ||
      msg.indexOf("বাবু") != -1 || msg.indexOf("সোনা") != -1 ||
      msg.indexOf("জান") != -1 || msg.indexOf("কিউট") != -1) {
    return LOVE;
  }

  // Peaceful/Angel keywords
  if (msg.indexOf("peace") != -1 || msg.indexOf("সান্তি") != -1 ||
      msg.indexOf("calm") != -1 || msg.indexOf("quiet") != -1 ||
      msg.indexOf("ভালো") != -1 || msg.indexOf("nice") != -1 ||
      msg.indexOf("লক্ষ্মী") != -1 || msg.indexOf("ভালো ছেলে") != -1 ||
      msg.indexOf("ভালো মেয়ে") != -1 || msg.indexOf("শান্ত") != -1) {
    return ANGEL;
  }

  // Funny keywords
  if (msg.indexOf("haha") != -1 || msg.indexOf("lol") != -1 ||
      msg.indexOf("funny") != -1 || msg.indexOf("joke") != -1 ||
      msg.indexOf("হাহা") != -1 || msg.indexOf("মজা") != -1) {
    return LAUGH;
  }

  // Sad/Bored keywords -> Leads to SLEEP if bored (annoyed)
  if (msg.indexOf("sad") != -1 || msg.indexOf("দুঃখ") != -1 ||
      msg.indexOf("cry") != -1 || msg.indexOf("কান্না") != -1 ||
      msg.indexOf("বিরক্ত") != -1 || msg.indexOf("bored") != -1) {
    return (msg.indexOf("বিরক্ত") != -1 || msg.indexOf("bored") != -1) ? SLEEP
                                                                      : SAD;
  }

  // Angry keywords
  if (msg.indexOf("angry") != -1 || msg.indexOf("রাগ") != -1 ||
      msg.indexOf("hate") != -1 || msg.indexOf("stupid") != -1 ||
      msg.indexOf("bad") != -1 || msg.indexOf("খারাপ") != -1) {
    return ANGRY_RAGE;
  }

  // Shocked/Surprised keywords
  if (msg.indexOf("wow") != -1 || msg.indexOf("omg") != -1 ||
      msg.indexOf("unbelievable") != -1 || msg.indexOf("অবাক") != -1 ||
      msg.indexOf("অবিশ্বাস্য") != -1) {
    return SHOCKED;
  }

  // Fear keywords
  if (msg.indexOf("scared") != -1 || msg.indexOf("ভয়") != -1 ||
      msg.indexOf("danger") != -1 || msg.indexOf("বিপদ") != -1) {
    return FEAR;
  }

  // Question/Curious keywords
  if (msg.indexOf("?") != -1 || msg.indexOf("কেন") != -1 ||
      msg.indexOf("কি") != -1 || msg.indexOf("why") != -1 ||
      msg.indexOf("what") != -1 || msg.indexOf("how") != -1) {
    return SKEPTICAL;
  }

  return EMO_NONE; // No strong sentiment detected
}
...
void handleChat() {
  ...
  Emotion userSentiment = detectChatSentiment(msg);
  if (userSentiment != EMO_NONE) {
    StimulusType stim = STIM_CHAT_POS;
    if (userSentiment == ANGRY_RAGE || userSentiment == SAD)
      stim = STIM_CHAT_NEG;
    else if (userSentiment == SHOCKED || userSentiment == FEAR)
      stim = STIM_CHAT_SURP;

    Reason.addStimulus(stim, "Chat: " + msg, 25.0);
    setEmotion(userSentiment, true); // Show reaction immediately
    Serial.print("[CHAT] User sentiment detected: ");
    Serial.println(userSentiment);
  }
...
void handleControl() {
  ...
  if (dir == "AUTO") {
    server.send(200, "text/plain",
                autoPilotActive ? "OFF" : "ON"); // Predict next state
