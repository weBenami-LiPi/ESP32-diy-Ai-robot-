...
private:
  ...
  float calculateBehaviorScore(BehaviorType type) {
    ...
    case BEHAVIOR_EXPLORE:
      score += boredomLevel * 0.4;
      score += energyLevel * 0.3;
      score += (moodLevel > 50 ? 0.2 : 0.0);
      score -= obstacleProximity * 0.3;
      score *= exploreWeight;
      break;

    case BEHAVIOR_SOCIAL:
      score += sensorFront * 0.5;
      score += moodLevel * 0.3;
      score += (100 - boredomLevel) * 0.2;
      score -= (100 - energyLevel) * 0.1;
      score *= socialWeight;
      break;

    case BEHAVIOR_REST:
      score += (100 - energyLevel) * 0.6;
      score += timeSinceActive * 0.3;
      score += (100 - moodLevel) * 0.1;
      score *= restWeight;
      break;

    case BEHAVIOR_ALERT:
      score += obstacleProximity * 0.7;
      score += (sensorFront > 0.8 ? 0.3 : 0.0); // Sudden detection
      score *= alertWeight;
      break;
    ...
    case BEHAVIOR_PLAY:
      score += moodLevel * 0.4;
      score += energyLevel * 0.3;
      score += (sensorFront * 0.3); // Play with person
      score -= boredomLevel * 0.2;
      score *= playWeight;
      break;
    }
