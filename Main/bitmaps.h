...
extern void setEmotion(Emotion e, bool isHighPriority = false);
...
inline void renderEmotionFrame(Adafruit_SSD1306 &d, Emotion emo, int frame) {
  ...
  case ANGEL: {
    ...
    int hCx = cx;
    int hCy = 10 - bnc;
    // 3D Halo Ring (Elliptical View)
    int haloRx = 22;
    int haloRy = 4;
    for (float i = 0; i < 6.28f; i += 0.15f) {
      int px = hCx + (int)(haloRx * cos(i));
      int py = hCy + (int)(haloRy * sin(i));
      d.drawPixel(px, py, SSD1306_WHITE);
      d.drawPixel(px, py + 1, SSD1306_WHITE); // Make it visible (2px thick)
    }

    // Rotating Sparkle (Synced with Ellipse)
    float rad = (frame * 0.2f);
    int sx = hCx + (int)(haloRx * cos(rad));
    int sy = hCy + (int)(haloRy * sin(rad));
  ...
  case GLITCH: {
    ...
    // 3. Random "Cracks" (Reduced to 1 per frame)
    int x1 = random(128);
    int y1 = random(64);
    d.drawLine(x1, y1, x1 + random(-10, 11), y1 + random(-10, 11),
               SSD1306_WHITE);

    // 4. Noise Pixels (Reduced)
    for (int n = 0; n < 5; n++) {
  ...
  // --- Living Scanline Layer (Reduced micro-flicker) ---
  if (frame % 4 == 0) {
    d.drawPixel(random(128), random(64), SSD1306_BLACK);
  }
