...
public:
  ...
  // Select emotion based on context and mood
  Emotion selectContextualEmotion(String context, float mood) {
    ...
    } else if (context == "CURIOUS") {
      return SKEPTICAL;
    } else if (context == "FOLLOW") {
      return (mood > 70) ? WINK : NEUTRAL;
    } else if (context == "HISS" || context == "SCARED") {
      return ANGRY_RAGE;
    }

    // Default mood-based selection for autonomous behavior
    if (mood > 80) {
      int choice = random(4);
      if (choice == 0)
        return PARTY;
  ...
  // Weighted autonomous emotion selector
  Emotion selectWeightedAutonomousEmotion(String context, float mood) {
    int roll = random(100);

    // 50% chance for NEUTRAL
    if (roll < AUTONOMOUS_NEUTRAL_WEIGHT) {
      return NEUTRAL;
    }

    // 5% chance for LOVE (if mood is good)
    if (roll < AUTONOMOUS_NEUTRAL_WEIGHT + AUTONOMOUS_LOVE_WEIGHT) {
      return (mood > 60) ? LOVE : WINK;
    }

    // 45% chance for OTHER emotions based on context
