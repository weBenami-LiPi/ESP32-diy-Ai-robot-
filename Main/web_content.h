...
#m{flex:1;background:var(--card);border-radius:12px;padding:12px;overflow-y:auto;margin-bottom:8px;border:1px solid var(--glass);display:flex;flex-direction:column;gap:6px}
...
.g{display:grid;grid-template-columns:repeat(4,1fr);grid-template-rows:repeat(5,1fr);gap:5px;padding:8px;background:rgba(0,0,0,0.2);border-radius:12px;width:100%;flex:1;margin:0;min-height:0}
.g button{width:100%;height:100%;font-size:24px;padding:0;background:var(--card);border:1px solid var(--glass);border-radius:8px;display:flex;justify-content:center;align-items:center}
...
.l{background:#000;color:#0f0;font:11px monospace;padding:10px;height:150px;overflow:auto;border-radius:10px;border:1px solid var(--glass);margin:5px 0}
...
<span>SYSTEM: ACTIVE</span><span style="color:var(--accent)">🔋 7700mAh [100%]</span></div>
...
<div class="split-wrap">
  <div class="joy-stick"> <!-- Horizontal Axis (Left) -->
  ...
  <div class="joy-stick"> <!-- Vertical Axis (Right) -->
...
<div id="lg" class="v"><div class="l" id="lc"></div><div class="l" id="ls"></div>
<button onclick="rl()" style="width:100%">REFRESH LOGS</button></div>
<script>
...
const rl=async()=>{document.getElementById('lc').innerText=await(await f('/chat_logs')).text();document.getElementById('ls').innerText=await(await f('/loc_logs')).text()};
...
window.onload=()=>{const e=document.getElementById('e');em.forEach((x,i)=>{const b=document.createElement('button');b.innerHTML=x;b.style.padding='8px';b.onclick=()=>f('/emotion?id='+i);e.appendChild(b)});ad('Vextor Online','b')};
