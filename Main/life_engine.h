...
public:
  ...
  void handleHeartbeat(unsigned long now) {
    if (!autoPilotActive)
      return; // No autonomous pulsing in manual mode
  ...
  void handleSocialAwareness(unsigned long now) {
    if (!autoPilotActive || (now - lastPresenceCheck < 200))
      return;
  ...
  void processPhysicalSensors(unsigned long now) {
    ...
    if (isPetting) {
      ...
      if (now >= persistenceEndTime || persistenceEndTime == 0) {
        // Enforce Strict Neutrality in Manual Mode:
        // Only allow petting emotions if Auto Pilot is active
        if (autoPilotActive) {
          if (currentEmotion != LOVE && currentEmotion != ANGEL) {
            if (currentMood > 80) {
              // User Request: 10% probability for LOVE + Cooldown Check
              extern unsigned long lastLoveTime;
              if (random(100) < 10 && (now - lastLoveTime >= 30000)) {
                setEmotion(LOVE, true);
              } else {
                setEmotion(ANGEL, true); // Friendly fallback
              }
            } else {
      ...
      Reason.addStimulus(STIM_TOUCHED, "User is petting me", 10.0);
  ...
  void detectTap(unsigned long startT, unsigned long now) {
    ...
    if (duration < 500) {
      ...
      if (hitCount >= 2) {
        ...
        Reason.addStimulus(STIM_CHAT_NEG, "I was hit twice!", 40.0);
        ...
        Reason.addStimulus(STIM_CHAT_NEG, "I was tapped", 10.0);
  ...
  void performIdleBehavior(unsigned long now) {
    ...
    // --- FIDGETING ---
    if (autoPilotActive && (now - lastFidget > random(3000, 8000))) {
      lastFidget = now;
    ...
    /*
      // DISABLED: User Request - No random autonomous expressions
    ...
    */
    setEmotion(NEUTRAL, false); // Always maintain Neutral in idle
  }
